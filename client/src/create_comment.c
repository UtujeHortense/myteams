...
void create_comment(req_s *myreq, int server_skt, path_t *myinfo)
{
    ...
    if (!myinfo->user_name)
    ...
    else if (get_arg_number(myreq->arg) != 1)
        printf("Invalid number of arguments.\n");
    else {
        myreq->arg = add_to_args(myreq->arg, myinfo->thread_title);
        ...
        if (!check_size(myreq->arg[0], MAX_BODY)) {
            printf("Error. Argument(s) size too long\n");
            return;
        }
        cmd = format_cmd_crlf("CANR", myreq->arg);
